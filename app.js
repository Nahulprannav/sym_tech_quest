(() => {
  ...
  function renderQuestionStation(station){
    ...
    let state=safeGet(`cipher:station:${station}`,null);
    if(!state || state.team!==profile.team){state={team:profile.team,ids:[],answers:{},complete:false};safeSet(`cipher:station:${station}`,state);}
    const redraw=()=>{
      if(!profile.team){root.innerHTML=`<div class="step-label">FIRST · TEAM REGISTRATION</div><h2>Who is entering the station?</h2><p>Enter your team name and choose the team designation assigned by the event coordinator.</p><div class="field-row"><div class="field"><label for="teamName">Team name</label><input id="teamName" placeholder="Your team name" value="${escapeHtml(profile.name)}" maxlength="32" required></div><div class="field"><label for="team">Assigned team</label><select id="team"><option value="">Choose Alpha–Epsilon…</option>${teams.map(t=>`<option value="${t}">Team ${t}</option>`).join('')}</select></div><button class="btn" id="begin">START STATION</button></div><p class="small" id="status"></p>`;$('#begin').onclick=()=>{const name=$('#teamName').value.trim(),team=$('#team').value;if(!name||!team){$('#status').textContent='Enter your team name and assigned team to continue.';return;}profile.name=name;profile.team=team;safeSet('cipher:profile',profile);state=safeGet(`cipher:station:${station}`,null);if(!state||state.team!==team){state={team,ids:[],answers:{},complete:false};safeSet(`cipher:station:${station}`,state);}redraw();};return;}
      if(!state.ids.length){state.ids=shuffle(banks[station-1].map((_,i)=>i)).slice(0,3);safeSet(`cipher:station:${station}`,state);}
      const answered=state.ids.filter(id=>state.answers[id]===banks[station-1][id].a).length;
      const qid=state.ids.find(id=>state.answers[id]!==banks[station-1][id].a);
      let body='';
      if(answered===3){if(isFinal){body=`<div class="step-label">FINAL CHECKPOINT CLEARED · 3 / 3</div><p class="success">All three questions are correct. Enter your four station keys to reveal your team’s final phrase.</p>`;}else{const secret=keys[profile.team][station-1];body=`<div class="step-label">STATION CLEARED · 3 / 3</div><p class="success">All three randomly assigned questions are correct. Your team key is ready.</p><div class="result show"><div class="step-label">TEAM ${profile.team.toUpperCase()} · KEY ${String(station).padStart(2,'0')}</div><div class="key-display">${secret}</div><div class="screenshot-note">📸 TAKE A SCREENSHOT OF THIS KEY AND SHOW YOUR COORDINATOR</div></div>`;}}
      else {const q=banks[station-1][qid];body=`<div class="step-label">TEAM ${escapeHtml(profile.name)} · ${profile.team.toUpperCase()} · QUESTION ${answered+1} OF 3</div><h2>${escapeHtml(q.q)}</h2><form id="answerForm"><div class="answer-list">${q.o.map((opt,i)=>`<label class="answer-option"><input type="radio" name="answer" value="${i}" required><span>${escapeHtml(opt)}</span></label>`).join('')}</div><button class="btn" type="submit">CHECK ANSWER</button></form><p id="feedback" class="small"></p><p class="screenshot-note">Five questions in this station’s pool · three selected for your team.</p>`;}
      ...
      const form=$('#answerForm');if(form)form.onsubmit=e=>{e.preventDefault();const selected=Number(new FormData(form).get('answer')),correct=selected===banks[station-1][qid].a;if(correct){state.answers[qid]=correct?selected:-1;safeSet(`cipher:station:${station}`,state);redraw();}else{$('#feedback').textContent='Not quite. Recheck the clue and try this question again.';$('#feedback').className='small error';}};
    };
    redraw();
  }
  function shuffle(a){for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}return a;}

  const decrypt=$('#decryptForm');if(decrypt)decrypt.addEventListener('submit',e=>{e.preventDefault();const t=$('#team').value;let ok=!!t;for(let i=0;i<4;i++)if($(`#key${i+1}`).value.trim().toUpperCase()!==keys[t||'Alpha'][i])ok=false;if(ok){$('#finalResult').innerHTML=`<p class="eyebrow">MASTER CIPHER DECRYPTED · TEAM ${t.toUpperCase()}</p><div class="final-phrase">${phrases[t]}</div><p class="success">MISSION COMPLETE · Tell a coordinator to record your finish time.</p>`;$('#finalResult').classList.add('show');}else{$('#finalResult').innerHTML='<p class="error">ACCESS DENIED · Check your team and all four keys against your screenshots.</p>';$('#finalResult').classList.add('show');}});
  const baseInput=$('#baseUrl');if(baseInput){baseInput.value=location.origin==='null'?'':location.origin+location.pathname.replace(/organizer\.html$/,'');$('#makeQR').addEventListener('click',()=>{const base=$('#baseUrl').value.trim().replace(/\/$/,'');const warning=$('#qrWarning');if(!/^https?:\/\//i.test(base)){warning.textContent='Enter a deployed public base URL (https://...) first. Local file URLs cannot be scanned by other phones.';warning.className='error';return;}if(!window.QRCode){warning.textContent='QR library did not load. Check internet access and reload this organizer page.';warning.className='error';return;}document.querySelectorAll('.qr-box').forEach((box,i)=>{box.innerHTML='';const url=`${base}/station${i+1}.html`;new QRCode(box,{text:url,width:150,height:150,colorDark:'#101827',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M});const link=document.createElement('a');link.className='download-qr';link.textContent='Download QR PNG';link.download=`project-cipher-station-${i+1}.png`;link.href=box.querySelector('canvas')?.toDataURL('image/png')||'';box.nextElementSibling.replaceChildren(link,document.createElement('br'),document.createTextNode(url));});warning.textContent='Ready. Download or print each code and place it at the matching station.';warning.className='success';});}
})();
