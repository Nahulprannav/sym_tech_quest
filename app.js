(() => {
  ...
  const supabaseUrl=(qrParams.get('db')||'').replace(/\/$/,'');
  const supabaseKey=qrParams.get('key')||'';
  const reportCompletion=(station,score,extra={})=>{
    ...
    const row={event_id:record.eventId,team_name:record.teamName,assigned_team:record.assignedTeam,station:record.station,score:record.score,status:record.status||(station<5?'station complete':'checkpoint complete'),key:record.key||null,final_phrase:record.finalPhrase||null,completed_at:record.completedAt};
    try{fetch(`${supabaseUrl}/rest/v1/completions?on_conflict=event_id`,{method:'POST',headers:{apikey:supabaseKey,'Content-Type':'application/json',Prefer:'resolution=ignore-duplicates,return=minimal'},body:JSON.stringify(row),keepalive:true}).then(r=>{if(!r.ok)console.warn('Project Cipher result could not be saved:',r.status);}).catch(err=>console.warn('Project Cipher result could not be saved:',err));}catch{}
  };
  ...
  const baseInput=$('#baseUrl');if(baseInput){const cfg=(()=>{try{return JSON.parse(localStorage.getItem('cipher:supabase-config')||'{}')}catch{return {}}})();baseInput.value=location.origin==='null'?'':location.origin+location.pathname.replace(/organizer\.html$/,'');$('#supabaseUrl').value=cfg.url||'';$('#supabaseKey').value=cfg.key||'';$('#makeQR').addEventListener('click',()=>{const base=$('#baseUrl').value.trim().replace(/\/$/,'');const db=$('#supabaseUrl').value.trim().replace(/\/$/,'');const key=$('#supabaseKey').value.trim();const warning=$('#qrWarning');if(!/^https?:\/\//i.test(base)){warning.textContent='Enter a deployed public base URL (https://...) first.';warning.className='error';return;}if((db||key)&&(!/^https:\/\/[\w-]+\.supabase\.co$/i.test(db)||!key)){warning.textContent='Enter both the Supabase project URL and its public anon key.';warning.className='error';return;}if(!window.QRCode){warning.textContent='QR library did not load. Check internet access and reload this organizer page.';warning.className='error';return;}try{localStorage.setItem('cipher:supabase-config',JSON.stringify({url:db,key}));}catch{}document.querySelectorAll('.qr-box').forEach((box,i)=>{box.innerHTML='';const target=new URL(`${base}/station${i+1}.html`);if(db){target.searchParams.set('db',db);target.searchParams.set('key',key);}const url=target.toString();new QRCode(box,{text:url,width:190,height:190,colorDark:'#101827',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M});const link=document.createElement('a');link.className='download-qr';link.textContent='Download QR PNG';link.download=`project-cipher-station-${i+1}.png`;link.href=box.querySelector('canvas')?.toDataURL('image/png')||'';box.nextElementSibling.replaceChildren(link,document.createElement('br'),document.createTextNode(url));});warning.textContent=db?'Ready. Station completions will be saved to Supabase. Download or print these new codes.':'Ready. QR codes work without a shared database until you add Supabase credentials.';warning.className=db?'success':'small';});}

  const login=$('#organizerLogin');
  if(login){
    const result=$('#organizerStatus'),table=$('#completionTable'),refresh=$('#refreshResults'),logout=$('#organizerLogout');let accessToken='';
    const config=()=>{try{return JSON.parse(localStorage.getItem('cipher:supabase-config')||'{}')}catch{return {}}};
    const loadRows=async()=>{const c=config();if(!accessToken||!c.url||!c.key)return;result.textContent='Loading completions…';try{const response=await fetch(`${c.url}/rest/v1/completions?select=team_name,assigned_team,station,score,status,key,final_phrase,completed_at&order=completed_at.desc`,{headers:{apikey:c.key,Authorization:`Bearer ${accessToken}`}});if(!response.ok)throw new Error('Could not read results. Check the database setup and organizer login.');const rows=await response.json();table.innerHTML=`<thead><tr><th>Completed</th><th>Team name</th><th>Assigned team</th><th>Station</th><th>Score</th><th>Status</th><th>Key / phrase</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${escapeHtml(new Date(r.completed_at).toLocaleString())}</td><td>${escapeHtml(r.team_name)}</td><td>${escapeHtml(r.assigned_team)}</td><td>${r.station}</td><td>${r.score}/3</td><td>${escapeHtml(r.status)}</td><td>${escapeHtml(r.final_phrase||r.key||'—')}</td></tr>`).join('')}</tbody>`;result.textContent=`${rows.length} completion record${rows.length===1?'':'s'} · refreshed just now.`;}catch(err){result.textContent=err.message;}};
    login.addEventListener('submit',async e=>{e.preventDefault();const c=config();if(!c.url||!c.key){result.textContent='Enter and save your Supabase project URL and anon key above, then generate the QR codes.';return;}result.textContent='Signing in…';try{const response=await fetch(`${c.url}/auth/v1/token?grant_type=password`,{method:'POST',headers:{apikey:c.key,'Content-Type':'application/json'},body:JSON.stringify({email:$('#organizerEmail').value.trim(),password:$('#organizerPassword').value})});const data=await response.json();if(!response.ok)throw new Error(data.msg||data.message||'Sign-in failed.');accessToken=data.access_token;login.hidden=true;$('#organizerSession').hidden=false;await loadRows();}catch(err){result.textContent=err.message;}});
    refresh.addEventListener('click',loadRows);logout.addEventListener('click',()=>{accessToken='';login.hidden=false;$('#organizerSession').hidden=true;table.replaceChildren();result.textContent='Signed out.';});
  }
})();
